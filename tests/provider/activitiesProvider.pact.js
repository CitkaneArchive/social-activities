...
describe('social-activities consumer expectations', async () => {
    const messageProviders = {};
    let persistanceMessages;
    before(async () => {
        const reqPath = `${pactBrokerUrl}/pacts/provider/social-persistance/consumer/social-activities/latest`;
        try {
    ...
    describe('add consumer requirement contracts to pact', () => {
        describe('CRUD read', () => {
            it('read.bffSubscriptions', () => {
                messageProviders['activities.read.bffSubscriptions'] = async (message) => {
        ...
        describe('CRUD create', () => {
            it('create.activity', async () => {
                let proxyResponse = persistanceMessages.find(message => message.description === 'persistance.create.activity');
                ...
                messageProviders['activities.create.activity'] = async (message) => {
                    const request = message.providerStates[0].name;
